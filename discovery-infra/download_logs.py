...
def main():
    ...
    if args.cluster_id:
        ...
        for cluster in clusters:
            cluster_events = client.get_events(cluster_id=cluster["id"], categories=["user", "metrics"])
...
def should_download_logs(cluster_events: list):
    for event in cluster_events:
        if ("Updated status of cluster" in event["message"] and "to error" in event["message"]) or ("Failed installing cluster" in event["message"]):
            return True
    return False
